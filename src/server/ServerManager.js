...
class ServerManager extends EventEmitter {
  ...
  async startServer(config, isRestore = false) {
    try {
      ...
      // Create mods directory for Forge servers
      if (config.type === 'forge') {
        await fs.ensureDir(path.join(serverDir, 'mods'));
        console.log('Created mods directory for Forge server');
      }
      ...
      // For Forge servers, create additional configuration files
      if (config.type === 'forge') {
        await this.setupForgeServer(serverDir, config);
      }
